...
from utils.common_utils.stopwatch import Stopwatch
from utils.common_utils.recorders import DatasetRecorder
from utils.common_utils import cprint, wrap_ruler
from torch.distributions.normal import Normal
from utils.common_utils.scaling_utils import *

# Import scaling utils
from utils.common_utils.scaling_utils import inverse_scaling
...
class EvalConfig:
    ...
    save_videos: bool = False  # New parameter to control video saving
    # output directory
...
def run_episode(
    ...
    success = False
    ever_grasped = False
    video_frames = []  # Store frames for video
    ...
    for step in range(max_len):
        with stopwatch.time("observe"):
            ...
            # Collect video frame if video saving is enabled
            if save_video and 'zed_sim_images' in raw_obs_dict:
                video_frames.append(raw_obs_dict['zed_sim_images'])
...
def run_evaluation(
    ...
    video_dir = None
    if cfg.save_videos:
        video_dir = Path(save_dir, f"eval_videos_epoch_{epoch_num}")
    ...
    for episode_num in range(cfg.num_episodes):
        ...
        # Prepare video path if saving videos
        video_path = None
        if cfg.save_videos and video_dir:
            success_prefix = ""  # Will be updated after episode completion
            video_path = video_dir / f"episode_{episode_num:03d}.mp4"
        
        if cfg.control_mode == "pd_ee_delta_pose":
            result = run_episode(
                policy, dataset, eval_env, cfg.freq, cfg.max_len, 
                stopwatch, cfg.show_camera, recorder, 
                use_tcp=cfg.use_tcp, save_video=cfg.save_videos, 
                video_path=str(video_path) if video_path else None
            )
        elif cfg.control_mode == "pd_ee_pose":
        ...
        if cfg.save_videos and video_path and video_path.exists():
            success_prefix = "success" if result["success"] else "failure"
            new_video_path = video_dir / f"{success_prefix}_episode_{episode_num:03d}.mp4"
            video_path.rename(new_video_path)
            print(f"Video renamed to: {new_video_path}")
    ...
    with open(stats_path, "w") as f:
        ...
        if cfg.save_videos and video_dir:
            f.write(f"Videos saved to: {video_dir}\n")
