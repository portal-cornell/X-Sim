...
class TopkSaver:
    ...
    def save(self, state_dict, perf, *, save_latest=False, force_save_name=None, config=None, epoch_num=None):
        ...
        if save_latest:
            weight_name = os.path.join(self.save_dir, f"{epoch_num}.pt" if (epoch_num is not None) else "latest.pt")
            torch.save(state_dict, weight_name)
            if config is not None:
                pickle.dump(config, open(f"{weight_name}.cfg", "wb"))
    ...
    def get_best_model(self):
        ...
        print(f"returning the best model {model_perf[0][1]} with score {model_perf[0][0]}")
