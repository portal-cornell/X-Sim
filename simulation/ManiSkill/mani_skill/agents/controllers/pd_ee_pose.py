...
class PDEEPoseController(PDEEPosController):
    ...
    def _inv_clip_and_scale_action(self, action):
        # Inverse of clipping and scaling for position
        pos_action = gym_utils.inv_clip_and_scale_action(
            action[:, :3], self.action_space_low[:3], self.action_space_high[:3]
        )
        
        # Inverse of rotation scaling
        rot_action = action[:, 3:].clone() / self.config.rot_lower
