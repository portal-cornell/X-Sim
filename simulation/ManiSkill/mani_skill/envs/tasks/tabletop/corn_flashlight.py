...
@register_env("Corn-Flashlight-Basket", max_episode_steps=100)
class CornFlashlightBasketEnv(BaseEnv):
    _sample_video_link = "https://github.com/haosulab/ManiSkill/raw/main/figures/environment_demos/PickCube-v1_rt.mp4"
    SUPPORTED_ROBOTS = ["panda", "panda_ninja_15", "fetch"]
    ...
    env_to_data_map = {
            "tabletop": {
                ...
                "robot_qpos": [0.0277, -0.3296, -0.0212, -1.9113,  0.0202,  1.6271,  0.7811, 0.04, 0.04],
                "camera": {
            ...
            "flat_kitchen": {
                ...
                # "robot_qpos": [0.0, -0.7853, -0.0, -2.3561, 0.0, 1.5707, 0.7853, 0.04, 0.04],
                "robot_qpos": [0.2611934, -0.4642156 ,  0.08966457, -2.3954022 ,  0.15271077, 2.0086708 ,  0.9597888, 0.04, 0.04],
                # "robot_qpos": [-0.05316861, -0.5721559 , -0.19464362, -2.3902714 , -0.21042612, 1.9439082 ,  0.74858224, 0.04, 0.04],
                "camera": {
    ...
    def __init__(
        ...
        self,
        *args,
        robot_uids="panda_ninja_15",
        robot_init_qpos_noise=0.02,
        randomize_init_config=False,
        obj_noise=0.0,
        environment_name="flat_kitchen",
        obj_names=['corn_new', 'basket_centered', 'flashlight'],  
        pick_place_idx=0,
        randomize_objects_list=[0, 2],
        demo_name='corn_in_basket_recalibrated',
        num_waypoints=5,
        first_waypoint_idx=70,
        last_waypoint_idx=170,
        goal_thresh = 0.05,
        angle_goal_thresh = 0.3,
        visualize=False,
        rotation_reward=True,
        require_grasp=True,
        **kwargs,
    ...
    def load_obj(self, scene, obj_file, name="obj", kinematic=False):
        ...
        builder.initial_pose = sapien.Pose()
        # print(kinematic, name)
        if (kinematic or name == "basket_centered" or name == "flashlight"):
    ...
    def _load_scene(self, options: dict):
        ...
        for i, obj_name in enumerate(self.obj_names):
            ...
            # Load the demo data
            if obj_name == "flashlight":
                pose_data = torch.zeros(175, 7).to(self.device)
                pose_data[:, :] = torch.tensor([0.54544739,  0.20791265,  0.19266673+0.02, -0.48532067, -0.47809097,0.55103002,  0.48193235]).to(self.device)
            else:
                pose_data = torch.tensor(np.load(demo_path)).float().to(self.device)[i]  
            
            # breakpoint()
            # np_data = np.load(demo_path)
            # flashlight_data = np.zeros_like(np_data[0])
            # Assumes just 1 object at index 0
            # pose_data[:, 2] += 0.02
            ...
            obj_scaling = torch.tensor([
                1 / max(torch.norm(obj_waypoints[j][:3] - obj_waypoints[j+1][:3], p=2).item(), 0.000001)
        ...
        # breakpoint()
        self.alternate_start_poses = [
            torch.tensor([ 0.5339,  0.2079,  0.2191, -0.3103, -0.5678,  0.6849, -0.3352]).to(self.device),
            # torch.tensor([ 0.5539,  0.3591,  0.2091, -0.3103, -0.5678,  0.6849, -0.3352]).to(self.device),
            torch.tensor([0.5469, -0.3530,  0.2577,  0.5203,  0.4871,  0.4920,  0.4999]).to(self.device),
            torch.tensor([0.5454,  0.3591,  0.2127, -0.4853, -0.4781,  0.5510,  0.4819]).to(self.device),
            # torch.tensor([0.5454,  0.2079,  0.2327, -0.4853, -0.4781,  0.5510,  0.4819]).to(self.device),
        ]

        self.standard_start_poses = [
            torch.tensor([ 0.5339,  0.3391,  0.2191, -0.3103, -0.5678,  0.6849, -0.3352]).to(self.device),
            torch.tensor([0.5469, -0.3530,  0.2577,  0.5203,  0.4871,  0.4920,  0.4999]).to(self.device),
            torch.tensor([0.5454,  0.2079,  0.2127, -0.4853, -0.4781,  0.5510,  0.4819]).to(self.device),
        ]
        # self.start_poses = self.alternate_start_poses
    ...
    def _initialize_episode(self, env_idx: torch.Tensor, options: dict):
        with torch.device(self.device):
            ...
            # generate mask that assigns 50% of episodes in random to alternate start poses
            ...
            for i, obj_mesh in enumerate(self.obj_meshes):
                ...
                xyz[:, :3] = self.standard_start_poses[i][:3]
                xyz[mask] = self.alternate_start_poses[i][:3]
                ...
                quat[:, ] = self.standard_start_poses[i][3:7]
                quat[mask] = self.alternate_start_poses[i][3:7]
                qs = torch.zeros((b, 4))
                qs[:, 0] = 1
    ...
    def _get_obs_extra(self, info: Dict):
        ...
        if "state" in self.obs_mode or True:
            obj_poses_list = []
            desired_goals_list = []
    ...
    def evaluate(self):
        ...
        for i, obj_mesh in enumerate(self.obj_meshes[:1]):
        ...
        return {
            "success": is_obj_placed[:, 0] & is_obj_rotated[:, 0] & is_robot_static,
