...
from mani_skill.envs.tasks.tabletop.corn_flashlight import CornFlashlightBasketEnv
...
@register_env("Corn-Flashlight-Eval-viewpoints", max_episode_steps=100)
class CornFlashlightEvalViewEnv(CornFlashlightBasketEnv):
    """
    ...
    def _default_human_render_camera_configs(self):
        return CameraConfig(
            ...
            # fov=100,
            intrinsic=[
                [1.0663400e+03 * 0.8, 0.0000000e+00, 9.724600e+02],
                [0.0000000e+00, 1.0662000e+03 * 0.8, 5.229580e+02],
    ...
    def __init__(self, *args, randomize_lighting=False, randomize_camera=False, **kwargs):        
        ...
        self.lighting_configs = [
            ...
            #     },
            #     "fill_light": {"direction": [0.5, -0.3, -0.7], "color": [0, 0, 1]}
            # },
            # {
            #     "ambient": [0.1, 0.1, 0.9],
            #     "main_light": {
            ...
            #     },
            #     "fill_light": {"direction": [-0.4, 0.2, -0.6], "color": [0, 0, 0.3]}
            # },
            # {
            #     "ambient": [0.1, 0.1, 0.9],
            #     "main_light": {
            ...
            #         "shadow_map_size": 2048
            #     },
            #     "fill_light": {"direction": [0.05, -0.05, -1], "color": [1.7, 0.8, 0.8]}
            # },
            {
                "ambient": [0.5, 0.3, 1.1],
                "main_light": {
                ...
                "fill_light": {"direction": [0.05, -0.05, -1], "color": [0.5, 0.8, 1.1]}
            },
        ]
        ...
        self.default_eye = np.array([0.14, 0, 0.52])
        self.default_target = np.array([0.2, 0, 0.48])

        # self.default_eye = np.array([0.07, 0.65 - 0.15, 0.6 + 0.4])
        # self.default_target = np.array([0.4, 0.35 - 0.15, 0.4 + 0.1])
        
        # Define random variation ranges (in meters for position, radians for angle)
        self.pose_variation = {
            "eye_xy": 0.0,      # Max horizontal displacement
            "eye_z": 0.0,      # Max vertical displacement
            "target_xy": 0.0,  # Max target point horizontal shift
            "target_z": 0.0   # Max target point vertical shift
        }
        
        super().__init__(*args, **kwargs)
        self.alternate_start_poses[-1][2] += 2
        self.standard_start_poses[-1][2] += 2
    ...
    def evaluate(self):
        ...
        info = super().evaluate()
        # breakpoint()
        # info["success"] = self.current_subgoal_idx >= 5
        # print(f"Success: {info['success']}")
        # print(f"Current subgoal idx: {self.current_subgoal_idx}")
        ...
        for i, obj_mesh in enumerate(self.obj_meshes):
            if i == self.single_obj_pick_place_idx:
                ...
                is_tcp_near_obj = tcp_obj_distance <= 0.07
        info.update({
