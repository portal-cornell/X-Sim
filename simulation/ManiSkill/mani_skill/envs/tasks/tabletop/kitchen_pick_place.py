...
@register_env("Kitchen-PickPlace", max_episode_steps=50)
class KitchenPickPlaceEnv(BaseEnv):
    """
    **Task Description:**
    ...
    **Success Conditions:**
    ...
    def __init__(self, *args, robot_uids="panda_ninja", robot_init_qpos_noise=0.02, randomize_init_config=True, **kwargs):
        self.poses = torch.tensor(np.load(f"{PACKAGE_ASSET_DIR}/portal_lab/move_cube/poses.npy")).to('cuda:0')
        self.robot_init_qpos_noise = robot_init_qpos_noise
        self.randomize_init_config = randomize_init_config
        self.convex_adjustment = -0.01
    ...
    def _default_human_render_camera_configs(self):
        ...
        # pose = sapien_utils.look_at([0.5, -1, 0.9], 
        #                             [0.5, 1, 0])
    ...
    def _load_object(self, scene):
        ...
        builder.initial_pose = sapien.Pose(p=[0.5730, 0.1690, 0.1888 + self.convex_adjustment])
    ...
    def _load_scene(self, options: dict):
        ...
        self.poses = torch.tensor(np.load(f"{PACKAGE_ASSET_DIR}/portal_lab/move_cube/poses.npy")).to('cuda:0')
        self.poses[:, 2] += self.cube_half_size
    ...
    def _initialize_episode(self, env_idx: torch.Tensor, options: dict):
        with torch.device(self.device):
            ...
            # ROBOT AND ENV SETUP
            self.mesh.set_pose(sapien.Pose(p=[0.5730, 0.1690, 0.1888 + self.convex_adjustment]))
            qpos = np.array(
            ...
            # OBJECT SETUP
            randomized_amt = torch.rand((b, 2)) * 0.02 - 0.01
            xyz = torch.zeros((b, 3))
            xyz += self.poses[-1][:3]
            xyz[:, :2] += randomized_amt
            self.cube.set_pose(Pose.create_from_pq(xyz))
            goal_xyz = torch.zeros((b, 3))
            goal_xyz += self.poses[-1][:3]
            goal_xyz[:, 2] += 0.15
            goal_xyz[:, :2] += randomized_amt
            self.goal_site.set_pose(Pose.create_from_pq(goal_xyz))


            # xyz = torch.zeros((b, 3))
            # xyz[:, :3] = torch.tensor([0.72, -0.275, 0.2313 + self.cube_half_size])
            # self.cube.set_pose(Pose.create_from_pq(xyz))
            # goal_xyz = torch.zeros((b, 3))
            # goal_xyz[:, :3] = torch.tensor([0.6577, 0.2093, 0.2030 + self.cube_half_size])
            # self.goal_site.set_pose(Pose.create_from_pq(goal_xyz))
