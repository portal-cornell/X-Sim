...
@register_env("PickCube-v1-custom", max_episode_steps=50)
class PickCubeCustomEnv(BaseEnv):
    """
    **Task Description:**
    ...
    **Success Conditions:**
    ...
    def __init__(self, *args, robot_uids="panda", robot_init_qpos_noise=0.02, randomize_init_config=True, **kwargs):
    ...
    def _initialize_episode(self, env_idx: torch.Tensor, options: dict):
        with torch.device(self.device):
            ...
            if not self.randomize_init_config:
                xyz[:, :2] = torch.zeros((b, 2))
            ...
            goal_qs = None
            if not self.randomize_init_config:
                goal_xyz[:, :2] = torch.zeros((b, 2)) + 0.1
                goal_xyz[:, 2] = 0.3 + xyz[:, 2]
                # goal_qs = torch.tensor([[.707, 0, 0]])
                # goal_qs = axis_angle_to_quaternion(goal_qs)
    ...
    def evaluate(self):
        ...
        # is_obj_rotated = (
        #     torch.rad2deg(quaternion_angle(self.goal_site.pose.q, self.cube.pose.q))
        #     <= 3
        # )
    ...
    def compute_dense_reward(self, obs: Any, action: torch.Tensor, info: Dict):
        ...
        # rot_reward = -(torch.abs(quaternion_angle(self.goal_site.pose.q, self.cube.pose.q)) / np.pi)
