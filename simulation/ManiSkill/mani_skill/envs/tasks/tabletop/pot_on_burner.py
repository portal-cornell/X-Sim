...
@register_env("Pot-On-Burner", max_episode_steps=600)
class PotOnBurnerEnv(BaseEnv):
    ...
    def __init__(
        ...
        *args,
        ...
        obj_names=['blue_pot_new', 'burner'],  
        pick_place_idx=0,
        randomize_objects_list=[0],
        demo_name='pot_rim',
        # demo_name='pot_on_burner',
        num_waypoints=5,
        first_waypoint_idx=39,
        last_waypoint_idx=107,
        goal_thresh = 0.04,
        ...
        **kwargs,
        ...
        # obj_noise=0.0,
        # environment_name="flat_kitchen",
        # obj_names=['mustard_centered'],  
        # pick_place_idx=0,
        # randomize_objects_list=[0],
        # demo_name='mustard_rl',
        # num_waypoints=5,
        # first_waypoint_idx=89,
        # last_waypoint_idx=205,
        # goal_thresh = 0.01,
        # angle_goal_thresh = 0.15,
        # visualize=False,
        # rotation_reward=True,
        # require_grasp=True,
        # **kwargs,
    ...
    def _load_scene(self, options: dict):
        ...
        for i, obj_name in enumerate(self.obj_names):
            ...
            if i == self.single_obj_pick_place_idx:
                ...
                obj_mesh = self.load_obj(self.scene, obj_file, name=obj_name, kinematic=True)
            ...
            pose_data[:,2] += 0.02
            # If this is the first object (i==0), rotate all poses by 180 degrees around Z-axis
            if i == 0:
                # Create a quaternion for 180-degree rotation around Z-axis
                z_rotation_quat = axis_angle_to_quaternion(
                    torch.tensor([0, 0, np.pi], device=self.device),
                )
                
                # Apply the rotation to all pose quaternions
                for j in range(len(pose_data)):
                    original_quat = pose_data[j, 3:7]
                    rotated_pose = Pose.create_from_pq(q=z_rotation_quat) * Pose.create_from_pq(q=original_quat)
                    pose_data[j, 3:7] = rotated_pose.q
                # also add 10 cm to the y direction
                pose_data[:, 1] += 0.1
    ...
    def evaluate(self):
        ...
        is_obj_placed2 = torch.zeros((self.num_envs, num_objects), dtype=torch.bool, device=self.device)
        is_obj_rotated2 = torch.zeros((self.num_envs, num_objects), dtype=torch.bool, device=self.device)
        ...
        all_objects_placed2 = torch.all(is_obj_placed2, dim=1)
        all_objects_rotated2 = torch.all(is_obj_rotated2, dim=1)

        return {
            "success": all_objects_placed2 & all_objects_rotated2 & is_robot_static,
    ...
    def compute_dense_reward(self, obs: Any, action: torch.Tensor, info: Dict):
        ...
        if self.require_grasp:
            reward += info['is_grasped'][torch.arange(self.num_envs, device=self.device), self.current_obj_to_manip_idx] * reaching_reward
