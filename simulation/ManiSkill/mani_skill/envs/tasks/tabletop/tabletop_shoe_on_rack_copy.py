...
@register_env("Tabletop-Shoe-On-Rack-Copy", max_episode_steps=200)
class TabletopShoeOnRackCopyEnv(BaseEnv):
    ...
    env_to_data_map = {
            "tabletop": {
                "kitchen_to_robot_transform": [0.2060+0.012 - 0.02, 0.7336, 0.3448 - 0.015 + 0.01],
                "robot_qpos": [0.0109, -0.6258,  0.0274, -1.9208,  0.0732,  1.2061,  0.8767, 0.04, 0.04],
                # "robot_qpos": [-0.2220, -0.0843, 0.1698, -1.8714, 0.1572, 1.7793, 0.6444, 0.04, 0.04],
                "camera": {
                    "eye": [0.10742855, 0.5960838, 1.11373567],
                    "target": [0.68296728, 0.14237866, 0.43336311]
                },
                "ground_altitude": -0.7,
            },
            "flat_kitchen": {
                "kitchen_to_robot_transform": [0.4147, 0.1753 - 0.01, 0.1875 - 0.01],
                "robot_qpos": [0.0, -0.7853, -0.0, -2.3561, 0.0, 1.5707, 0.7853, 0.04, 0.04],
                "camera": {
                    "eye": [-0.03740435, -0.45771217,  0.61309804],
                    "target": [0.77913187, -0.02136986,  0.23511127]
                },
                "ground_altitude": -0.6,
            }
        }
    ...
    def __init__(
        ...
        *args,
        ...
        obj_noise=0.0,
        environment_name="tabletop",
        obj_names=['left_shoe_edge', 'right_shoe', 'shoe_rack'],  
        pick_place_idx=0,
        randomize_objects_list=[0],
        demo_name='higher_new',
        num_waypoints=1,
        first_waypoint_idx=47,
        last_waypoint_idx=100,
        goal_thresh = 0.05,
    ...
    def load_obj(self, scene, obj_file, name="obj", kinematic=False):
        ...
        builder.initial_pose = sapien.Pose()
        if kinematic or name == "shoe_rack":
    ...
    def _load_scene(self, options: dict):
        ...
        for i, obj_name in enumerate(self.obj_names):
            ...
            # breakpoint()
            if obj_name == "left_shoe_edge":
                pose_data[0, :3] += torch.tensor([0.0, 0.0, 0.01], device=self.device)
            ...
            if i == 0:
                self.goal_poses.append(torch.tensor([ 0.4549,  0.1446,  0.6165,  0.4583,  0.4928, -0.5156, -0.5304],device='cuda:0'))
            else:
                self.goal_poses.append(pose_data[self.end_idx])
    ...
    def evaluate(self):
        ...
        return {
            "success": is_obj_placed[:, i] & is_obj_rotated[:, i] & is_robot_static,
            "all_obj_placed": all_objects_placed,
            "all_obj_rotated": all_objects_rotated,
            "is_robot_static": is_robot_static,
            "is_grasped": is_grasped,
            "dist": torch.linalg.norm(self.goal_xyz[:, i, :3] - obj_mesh.pose.p[:, :3], axis=1),
            "angle": torch.abs(quaternion_angle(self.goal_quat[:, i], obj_mesh.pose.q))
        }
