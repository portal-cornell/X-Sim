...
@register_env("Tabletop-Subgoals", max_episode_steps=300)
class TabletopSubgoalsEnv(BaseEnv):
    """
    **Task Description:**
    ...
    **Success Conditions:**
    ...
    goal_thresh = 0.01
    angle_goal_thresh = 0.2 # 10 degrees
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.kitchen_to_robot_transform = torch.tensor([0.2060, 0.7336, 0.3448])
    ...
    def _default_human_render_camera_configs(self):
        pose_new = sapien_utils.look_at([0.95304242, -0.07714972,  0.97090369], 
    ...
    def _load_scene(self, options: dict):
        self.ground = build_ground(self.scene, altitude=-0.7)
        ...
        # last one is one where object should not be grasped
        self.waypoints.append(self.poses[self.end_idx])
    ...
    def _initialize_episode(self, env_idx: torch.Tensor, options: dict):
        with torch.device(self.device):
            ...
            qpos = np.array(
                [0.0277, -0.3296, -0.0212, -1.9113,  0.0202,  1.6271,  0.7811, 0.04, 0.04]
            )
    ...
    def _get_obs_extra(self, info: Dict):
        ...
        if "state" in self.obs_mode:
            ...
            # obs["is_obj_static"] = info['is_obj_static']
            # obs["is_obj_rotated"] = info['is_obj_rotated']
            # obs["qpos"] = self.agent.robot.get_qpos()
            # obs["qvel"] = self.agent.robot.get_qvel()
