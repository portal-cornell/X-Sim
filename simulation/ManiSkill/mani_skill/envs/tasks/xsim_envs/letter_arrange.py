...
@register_env("Letter-Arrange", max_episode_steps=50)
class LetterArrangeEnv(BaseEnv):
    ...
    def _load_demo_data(self):
        ...
        for i, obj_name in enumerate(self.obj_names):
            ...
            obj_mesh = self._load_obj(self.scene, obj_file, name=obj_name)
    ...
    def _initialize_episode(self, env_idx: torch.Tensor, options: dict):
        with torch.device(self.device):
            ...
            self.current_obj_to_manip_idx[env_idx] = self.single_obj_manip_idx
    ...
    def evaluate(self):
        ...
        for i, obj_mesh in enumerate(self.obj_meshes):
            ...
            is_obj_placed[:, i] = (
                torch.linalg.norm(self.goal_xyz[:, i, :3] - obj_mesh.pose.p[:, :3], axis=1) <= 0.03
            )
            
            # Check rotation
            is_obj_rotated[:, i] = (
                torch.abs(quaternion_angle(self.goal_quat[:, i], obj_mesh.pose.q)) <= 0.15
            ) 
    ...
    def _compute_waypoint_reward(self, info: Dict):
        ...
        # Progress bonus
        waypoint_reward += obj_current_idx.float()
