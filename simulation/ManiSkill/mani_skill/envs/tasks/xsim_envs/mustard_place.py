...
@register_env("Mustard-Place", max_episode_steps=200)
class MustardPlaceEnv(BaseEnv):
    SUPPORTED_ROBOTS = ["panda", "panda_ninja_15"]
    agent: Union[Panda]
    ...
    def evaluate(self):
        ...
        for i, obj_mesh in enumerate(self.obj_meshes):
            ...
            is_obj_placed[:, i] = (
                torch.linalg.norm(self.goal_xyz[:, i, :3] - obj_mesh.pose.p[:, :3], axis=1) <= 0.05
            )
            
            # Check rotation
            is_obj_rotated[:, i] = (
                torch.abs(quaternion_angle(self.goal_quat[:, i], obj_mesh.pose.q)) <= 0.3
            ) 
