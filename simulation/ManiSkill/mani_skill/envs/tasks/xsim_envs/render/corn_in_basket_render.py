...
from mani_skill.envs.tasks.xsim_envs.corn_in_basket import CornInBasketEnv
...
@register_env("Corn-in-Basket-Eval", max_episode_steps=200)
class CornInBasketEvalEnv(CornInBasketEnv):
    """
    Evaluation version of CornInBasketEnv that terminates episodes when success is reached.
    
    This environment extends the refactored CornInBasketEnv with lighting and camera randomization
    ...
    def evaluate(self):
        ...
        for i, obj_mesh in enumerate(self.obj_meshes):
            if i == self.single_obj_manip_idx:
                ...
                is_tcp_near_obj = tcp_obj_distance <= 0.04
        info.update({
