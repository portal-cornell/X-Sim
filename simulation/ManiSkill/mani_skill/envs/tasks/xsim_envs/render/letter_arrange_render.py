...
from mani_skill.envs.tasks.xsim_envs.letter_arrange import LetterArrangeEnv
...
@register_env("Letter-Arrange-Eval", max_episode_steps=100)
class LetterArrangeEvalEnv(LetterArrangeEnv):
    """
    Evaluation version of LetterArrangeEnv that terminates episodes when success is reached.
    
    This environment extends the refactored LetterArrangeEnv with lighting and camera randomization
    for evaluation purposes, while maintaining compatibility with the new modular structure.
    """
    ...
    def __init__(self, *args, randomize_lighting=False, randomize_camera=True, **kwargs):        
        # Store randomization flags
        self.randomize_lighting = randomize_lighting
        self.randomize_camera = randomize_camera
        
        # Define a list of lighting configurations with small variations from the default
        ...
        # Initialize parent class - task_name and environment_name are already defaults
        super().__init__(*args, **kwargs)

    def _initialize_episode(self, env_idx: torch.Tensor, options: dict):
        # Call parent class initialization
        super()._initialize_episode(env_idx, options)
        default_eye = np.array(self.env_config['camera']['eye'])
        default_target = np.array(self.env_config['camera']['target'])
        ...
        # Handle camera pose
        for i in range(b):
            pose = sapien_utils.look_at(default_eye, default_target)
