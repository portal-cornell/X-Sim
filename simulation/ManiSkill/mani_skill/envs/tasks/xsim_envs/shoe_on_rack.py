...
@register_env("Shoe-on-Rack", max_episode_steps=200)
class ShoeOnRackEnv(BaseEnv):
    ...
    def __init__(
        ...
        *args,
        ...
        obj_noise=0.0,
        environment_name="tabletop_env",
        task_name="shoe_on_rack",
        visualize=False,
    ...
    def _load_demo_data(self):
        ...
        for i, obj_name in enumerate(self.obj_names):
            ...
            # Special adjustment for left_shoe
            if obj_name == "left_shoe":
            ...
            # Special goal pose for left_shoe
            if i == 0:
                self.goal_poses.append(torch.tensor(
                    [0.4549, 0.1446, 0.6165, 0.4583, 0.4928, -0.5156, -0.5304], 
                    device=self.device
                ))
            else:
    ...
    def _setup_waypoints(self):
        ...
        for i, pose_data in enumerate(self.poses):
            ...
            # Special waypoint selection for this task
            for j in [47, 54, 99]:
    ...
    def evaluate(self):
        ...
        # Check if this object is at its goal position (XY only)
        is_obj_placed[:, i] = (
            torch.linalg.norm(self.goal_xyz[:, i, :3] - obj_mesh.pose.p[:, :3], axis=1) <= 0.02
        )
        
        is_obj_rotated[:, i] = (
            torch.abs(quaternion_angle(self.goal_quat[:, i], obj_mesh.pose.q)) <= 0.2
        ) 
    ...
    def _compute_waypoint_reward(self, info: Dict):
        ...
        if self.rotation_reward:
            ...
            angle_reward = 1 - torch.tanh(angle_to_current)
