...
class CustomTableSceneBuilder(SceneBuilder):
    def build(self):
    ...
    def initialize(self, env_idx: torch.Tensor):
        ...
        b = len(env_idx)
        self.table.set_pose(
            sapien.Pose(p=[-0.12 + 0.615 + 0.6, 0, -0.9196429 + 0.1875 - 0.01], q=euler2quat(0, 0, np.pi / 2))
        )
        if self.env.robot_uids == "panda":
        ...
        elif self.env.robot_uids == "panda_ninja":
            qpos = np.array(
                # [
                #     0.0,
                #     np.pi / 8,
            ...
            qpos[:, -2:] = 0.04
            self.env.agent.reset(qpos)
            self.env.agent.robot.set_pose(sapien.Pose([0, 0, 0]))
