...
if __name__ == "__main__":
    ...
    # env setup
    env_kwargs = dict(obs_mode="state", render_mode="rgb_array", sim_backend="gpu", reward_mode=args.reward_mode, randomize_init_config=args.randomize_init_config, obj_noise=args.obj_noise)
    ...
    if args.capture_video:
        ...
        if args.save_train_video_freq is not None:
            ...
            envs = RecordEpisode(envs, output_dir=f"runs/{run_name}/train_videos", save_trajectory=False, save_video_trigger=save_video_trigger, max_steps_per_video=args.num_steps, video_fps=5)
        eval_envs = RecordEpisode(eval_envs, output_dir=eval_output_dir, save_trajectory=args.evaluate, trajectory_name="trajectory", max_steps_per_video=args.num_eval_steps, video_fps=5)
    ...
    if not args.evaluate:
        ...
        if args.track:
            ...
            config["eval_env_cfg"] = dict(**env_kwargs, num_envs=args.num_eval_envs, env_id=args.env_id, env_horizon=max_episode_steps, partial_reset=False)
            wandb.init(
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        if iteration % args.eval_freq == 1:
            ...
            num_episodes = 0
            eval_plots_dict = defaultdict(list)
            for _ in range(args.num_eval_steps):
                with torch.no_grad():
                    eval_obs, eval_rew, eval_terminations, eval_truncations, eval_infos = eval_envs.step(agent.get_action(eval_obs, deterministic=True))
                    if args.evaluate and args.num_eval_envs == 1:
                        for key in eval_infos['extra_data']:
                            eval_plots_dict[key].append(eval_infos['extra_data'][key].item())
            ...
            if args.evaluate and args.num_eval_envs == 1:
                ...
                # Plot each key's statistics on the same plot
                for key in eval_plots_dict:
                    plt.plot(eval_plots_dict[key], label=key)
                ...
                plt.savefig(f"eval_plots_{global_step}.png")
                break
